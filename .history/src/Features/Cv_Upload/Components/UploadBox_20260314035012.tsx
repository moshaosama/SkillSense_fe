import { Link2, Sparkles, Upload, X, FileText, CheckCircle2, Code2, RefreshCw } from "lucide-react";
...
const UploadBox = ({ onNext, onUploadSuccess, hasUploadedPdf = false }: { onNext: () => void; onUploadSuccess?: () => void; hasUploadedPdf?: boolean }) => {
      ...
      if (data.success === false) {
        ...
        setLoadingStage(loadingSteps.length - 1);
          setTimeout(() => {
            setIsLoading(false);
            setLoadingStage(0);
            toast.success(replaceMode ? "تم استبدال السيرة بنجاح!" : "File uploaded successfully!");
      ...
      <AnimatePresence mode="wait">
        {hasUploadedPdf && !replaceMode && (
          ...
          >
            ...
            <div>
              <h3 className="text-xl font-black text-slate-900 mb-1">تم رفع السيرة الذاتية</h3>
              <p className="text-sm font-bold text-slate-600">لو عايز تبدّل الملف، اضغط «استبدال»</p>
            </div>
            <motion.button
              type="button"
        ...
        {showReplaceModal && (
          ...
          >
            ...
            >
              ...
              <p className="text-slate-600 font-bold text-sm mb-6">عايز تستبدل السيرة الذاتية الحالية بملف جديد؟</p>
